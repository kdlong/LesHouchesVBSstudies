...
\end{alignat}
%
The numerical values used in the simulation for the pole mass/width of the gauge bosons ($V={\rm W,Z}$) are obtained from the measured on-shell (OS) values for the masses and widths according to \cite{Bardin:1988xt}:
%
\begin{equation}
...
For contributions (interference or QCD-induced contributions) where there is a dependence on $\alpha_{\rm s}$, the numerical value used is the one (potentially dynamically) extracted from the PDF.
...
\item All charged leptons are required to have
    ...
    \end{align}
\item For the leptons of opposite charge and same flavour, an invariant mass cut to single out the Z resonance is applied:
    \begin{align}
        \label{cut:2}
...
\item QCD jets are clustered thanks to the anti-$k_T$ algorithm~\cite{Cacciari:2008gp} with radius parameter $R=0.4$.
      At least two jets are required to have
        ...
        \end{align}
\item On the two leading jets fulfilling the criteria above, typical VBS cuts are applied:
        \begin{align}
        \label{cut:4}
