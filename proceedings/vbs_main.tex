...
We stress that such differences of configuration should be considered in addition to typical estimates of theoretical uncertainties, especially QCD scale PDF uncertainties. Two illustrate this, we compute the PDF uncertainty for the NNPDF3.0 set considered and the scale two scale choices considered here. The PDF uncertainty is evaluated to be 3--5\% using {\sc MG5\_aMC}. Scale uncertainties are evaluated using the typical prescription of varying $\mu_{R}$ and $\mu_{F}$ subject to the constraint $1/2 \le \mu_{F}/\mu_{R} \le 2$, using {\sc MG5\_aMC} and cross-checked with \MoCaNLO+\Recola, and are found to be between 7--10\% for the scale choices considered. The full results obtained with {\sc MG5\_aMC} are shown in Table~\ref{table:xsecWithUnc}
