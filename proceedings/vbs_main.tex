...
The predictions are not all in statistical agreement but they usually agree within about one per.
For the fixed scale displayed in Table~\ref{table:xsectLOfix}, the largest difference is between the predictions of \MoCaNLO\!+\Recola and \Sherpa and amounts to $1.2\%$.
Note also that the predictions of \MoCaNLO\!+\Recola and \VBFNLO are not expected to agree.
The predictions of \VBFNLO rely on the VBS approximation for the matrix element.
The difference between the two amounts to $0.6\%$ for the fixed scale.
...
\begin{tabular}{ c | c | c }
 $\mu = \mu_{\rm fix}$ / $\sigma_{\rm LO}^{\rm EW}$ [fb] & ${\rm p} {\rm p} \to {\rm e}^+  \nu_{\rm e}  \mu^+ \mu^- {\rm j} {\rm j}$  & ${\rm p} {\rm p} \to {\rm e}^-  \bar \nu_{\rm e}  \mu^+ \mu^- {\rm j} {\rm j}$  \\
  \hline\hline
  {\sc MG5\_aMC}                  & $0.2857(8)\phantom{0}$     & $0.1657(4)\phantom{0}$   \\
  {\sc MoCaNLO}+{\sc Recola}      & $0.28496(6)$               & $0.16718(3)$  \\
  {\sc Sherpa}                    & $0.2885(2)\phantom{0}$     & $0.1670(2)\phantom{0}$   \\
  {\sc VBFNLO}                    & $0.2867(5)\phantom{0}$     & $0.1661(3)\phantom{0}$   \\
  \hline
\end{tabular}
\end{center}
\caption{
...
for the jet rapidity separation, all predictions agree within statistical uncertainties over the whole range, withe some small normalization differences. For the di-jet mass, statistically significant differences are more pronounced towards higher masses between {\sc MG5\_aMC} and other generators. This difference was not fully understood, but could be attributed to undestimation of statistical uncertainties or subtle configuration differences.
...
\caption{Differential distributions computed at fixed-order with centre-of-mass energy $\sqrt{s}=13{\rm~TeV}$ at the LHC for ${\rm p} {\rm p}
  \to {\rm e}^-  \nu_{\rm e}  \mu^+ \mu^- {\rm j} {\rm j}$ at LO with fixed scaled $\mu = M_{\rm W}$: 
                ...
                The predictions in the lower plot are normalised to the prediction of {\sc MG5\_aMC}.
								...
								The statistical uncertainty on the {\sc MG5\_aMC} sample is shown in grey, other samples
...
We stress that such differences of configuration should be considered inpependently to typical estimates of theoretical uncertainties, especially QCD scale and PDF uncertainties. 
To illustrate this, we compute the PDF uncertainty for the NNPDF3.0 set and the two scale choices considered here. 
The PDF uncertainty is evaluated to be 3--5\% using {\sc MG5\_aMC}. 
Scale uncertainties are evaluated using the typical prescription of varying $\mu_{R}$ and $\mu_{F}$ 
subject to the constraint $1/2 \le \mu_{F}/\mu_{R} \le 2$, using {\sc MG5\_aMC} and cross-checked with \MoCaNLO+\Recola, 
and are found to be between 7--10\% for the scale choices considered. The full results obtained with {\sc MG5\_aMC} are shown in Table~\ref{table:xsecWithUnc}
...
We also compare the effect of configurations changes using the general-purpose generators {\sc Sherpa} and {\sc MG5\_aMC}. 
...
Using the same fiducial defintion, but with the default parameter settings (defined in param\_card.dat) for 
{\sc MG5\_aMC}, we obtain a cross section of 0.1561(5) fb for the ${\rm Max}\left[p_{\rm T, j}\right]$
scale, an increase of 4\% on our nominal value. The primary source of this difference is the 
settings of the boson masses and widths, which are set to their LO values by default in {\sc MG5\_aMC}.
...
more appropriate. We do not believe that this 4\% difference constitues an additional uncertainty, but warn that
careful configuration of such parameters should be considered.

We similarly study the default dynamical scale choices in {\sc Sherpa} and {\sc MG5\_aMC}. In both cases,
this choice is motivated by a desire for broad application to a variety of processes. For {\sc MG5\_aMC},
the default scale depends on the order of the process and shower settings. For LO without
merging of parton multiplicities, the scale is set to the central $m_{T}$ scale after k$_t$-clustering of the event.
...
of comparable size to the phenomonological uncertainties, should not necessarily be considered on equal footing.
The fact that the functional form of the dynamical scale choice can be signficant compared to the
...
This comparison additionally highlights the advantage of the reduced scale dependency at NLO.
