...
EB is supported by Chinese Academy of Sciences (CAS), MoST (Ministry of Science and Technology) through project no.~2013CB837801, and NSFC (National Natural Science Foundation of China) through project no.~11661141007. 
...
In addition, typical uncertainty treatments asses the impact of parton density function (PDF) 
uncertainties or of the missing higher orders in perturbative QCD. 
Monte Carlo generators require many input parameters,
...
induced by a variation of factorization, renormalization and
...
the fixed-order results after VBF cuts.
...
\KL{I put here what I used, Mathieu can update with the approach that gave better results...}
\begin{verbatim}
...
Beyond the di-jet invariant mass and the rapidity separation between the two jets, we have found that the distance between the two jets could have a good discriminating power.
We have also found that interferences between the QCD and EW amplitude are also negligible in the fiducial region.

In the present proceedings, we have also tried to perform a tuned comparison at LO.
...
These can probably be attributed to too aggressive error estimate or incorrect input for the runs presented here.
...
Concerning the predictions at LO supplemented with parton shower, they have relied on the fixed order comparison.
This means that no tuning of the parton shower parameters have been performed and the default values has been taken.
The agreement is in general worse.
In particular for observables that are defined only beyond LO, the theoretical predictions diverge significantly.
Concerning the role of the fixed and dynamical scale, we have found that they can have a rather large influence on the theoretical predictions at both the level of the cross section and differential distributions.
This obviously warrants the inclusion of higher order predictions that should cure this behaviour.
This is nonetheless beyond the scope of the present study.

Finally, we would like to stress that these results constitute only a preliminary study of theoretical predictions for ${\rm p}{\rm p} \to {\rm W}^{\pm}{\rm Z}{\rm j}{\rm j}$.
In particular, strictly fixed-order computation are expected to give a much better agreement.
At the level of parton shower, several differences appear and these are not yet fully understood.
In addition, NLO QCD and EW effect have not been included in this work.
It is obvious that these play a significant role in theoretical predictions for VBS.
They should be taken into account as much as possible in future studies of VBS.
