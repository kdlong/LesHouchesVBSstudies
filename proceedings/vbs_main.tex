...
EY is supported by CAS (Chinese Academy of Sciences), MoST (Ministry of Science and Technology), and NSFC (National Natural Science Foundation of China).
\label{vbs_section}}}
...
Qualitatively, one could expect a similar conclusion for other processes such as ${\rm W}^{\pm}{\rm Z}{\rm j}{\rm j}$ but a quantitative study is desirable.
...
\end{equation}
%
The predictions presented are all at LO but NLO QCD corrections to the EW contribution and its irreducible background are already known since 10 years in the VBS approximation \cite{Bozzi:2007ur} while the QCD corrections to the QCD-induced process have been computed more recently \cite{Campanario:2013qba}.
...
Concerning the interferences, as at the level of the cross section, they are also extremely suppressed at the level of differential distributions.
In particular, theirs contributions to each of the studied observables do not exceed the per-cent level over the whole phase space.
...
We have also found that interferences between the QCD and EW amplitudes are also negligible in the fiducial region.
...
These can probably be attributed to misconfiguration of the runs presented here.
...
For {\sc PYTHIA8}, the {\sc CUETP8M1} tune has been used \cite{Skands:2014pea}
...
Concerning the role of the fixed and dynamical scales, we have found that they can have a rather large influence on the predictions for the cross section and differential distributions.
